...
namespace SmartSchool.Utility.Helpers
{
    public static class TemplateGenerator
    {
        ...
        public static string BuildEmailVerificationBody(User model)
        {
            return $@"
            <html>
                <body style='font-family: Arial, sans-serif; font-size: 14px; color: #333; background-color: #f4f4f4; padding: 20px;'>
                    <div style='max-width: 600px; margin: 0 auto; background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);'>
                        ...
                        <p style='line-height: 1.6;'>
                            ...
                            <strong>Verification Code:</strong> {model.EmailOtp}<br />
                        </p>

                        <div style='margin-top: 30px; padding: 15px; background-color: #2e6c80; color: white; text-align: center; border-radius: 8px;'>
                            <p style='margin: 0;'>
                                <a href='http://localhost:4200/auth/verify?email={model.Person.Email}&token={model.EmailToken}' style='color: white; text-decoration: underline;'>Click Here to Verify Your Email</a>!
                            </p>
                        </div>
